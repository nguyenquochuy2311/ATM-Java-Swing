...
//            return true;
    ...
    public List<Transaction> ViewHistoryTransaction(BigInteger cardID) {
     ...
     public List<Transaction> ViewHistoryTransactionByTime(BigInteger cardID, int searchBy){
        String sql = null;
        switch(searchBy){
            // tat ca
            case 0:
                sql = "SELECT * FROM transaction where card_id = " +cardID;
                    break;
            // hom nay
            case 1:
                sql = "SELECT * FROM transaction where DATE(created_at) = CURDATE() and card_id = " +cardID;
                    break;
            // 1 tuan        
            case 2:
                sql = "SELECT * FROM transaction where YEARWEEK(created_at)=YEARWEEK(NOW()) and card_id = " +cardID;
                    break;
            //1 thang
            case 3:
                sql = "SELECT * FROM transaction where created_at > NOW() - interval 1 month and card_id = " +cardID;
                    break;
            case 4:
                sql = "SELECT * FROM transaction where created_at > DATE_SUB(NOW(),INTERVAL 1 YEAR) and card_id = " +cardID;
                    break;
            default:  break;
        }
        List<Transaction> listTran = new ArrayList<>();
